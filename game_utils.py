...
class game:
	...
	def __init__(self,path):
	    ...
	    self.unit_dict=self.load_dict('units.json')
	    self.names=self.load_dict('translate.json')
	    self.valid_moves=[]
	...
	def is_white(self,pos):
		return(self.board[pos].isupper())
	def is_black(self,pos):
		return(self.boad[pos].islower())
	...
	def draw_from_deck(self,player,pos):
		...
		new_name=self.name[self.deck[side][0]]
	...
	def move_unit(self,start,dest):
		...
		if self.board_arr[dest] != '_':
			self.wasted.append(board_arr[dest])
			...
			for move_type in u.moves[u.side].keys():
				...
				if move_type == 'm':
					...
					for tar in u.moves[u.side][move_type]:
						...
						while (check_dest != dest):
							check_dest=(check_dest[0]+line_cast[0],check_dest[1]+line_cast[1])
							if self.is_occupied(check_dest): hindered=True
						if (hindered): break
					...
					for direction in u.moves[u.side][move_type]:
						dest=tile_start
						while (not(dest[0]<0 and dest[0]>5 and dest[1]<0 and dest[1]>5) and ((self.is_occupied(dest)==False))):
					...
					for direction in u.moves[u.side][move_type]:
						dest=tile_start
						while (not(dest[0]<0 and dest[0]>5 and dest[1]<0 and dest[1]>5)): 
							#can keep location if is within bound, empty
							dest=(dest[0]+direction[0],dest[1]+direction[1])
					...
					for cmd in u.moves[u.side][move_type]:
						...
						if not(dest[0]<0 or dest[0]>5 or dest[1]<0 or dest[1]>5) and self.is_friendly(player,dest):
							...
							for friendly_move in vl:
								if (friendly_move[1:2]==self.position(coord=None,pair=dest)) and (friendly_move[3] != 'c'):
	...
	def execute_moves(self,player,move_str,flip=True):
		...
		if (move_str not in self.valid_moves):
			...
			if move_str[0]=='o':#spawn
				...
				init=self.position(coord=move_str[1:2],pair=None)
				if move_str[3]=='i':#command
					self.execute_moves(move_str[4:],flip=False)
				else:
					start=self.position(coord=move_str[1:2],pair=None)
					dest=self.position(coord=move_str[4:5],pair=None)
